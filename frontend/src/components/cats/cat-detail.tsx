...
export function CatDetail() {
  ...
  const { selectedCatId, setCurrentView, goBack, navigateToCreateEmergencyForCat } = useAppStore();
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        const data = await fetchCatById(selectedCatId!);
        setCat(data);
      } catch {
        setCat(null);
      } finally {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 space-y-5">
          ...
          <Button
            onClick={() => {
              navigateToCreateEmergencyForCat(cat.id);
            }}
            className="w-full"
          >
