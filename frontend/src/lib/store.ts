...
// --- donation progress cache ---
// a simple counter that increments whenever an admin approves/rejects a donation.
// any component that displays the fundraising total can watch this value and
// re-fetch when it changes, without needing a full page reload.
interface DonationProgressState {
  refreshTick: number;
  bumpRefreshTick: () => void;
}

export const useDonationProgress = create<DonationProgressState>((set) => ({
  refreshTick: 0,
  bumpRefreshTick: () => set((state) => ({ refreshTick: state.refreshTick + 1 })),
}));
