...
export async function fetchDonationProgress(): Promise<DonationProgress> {
  if (BACKEND_AVAILABLE) {
    const res = await fetch(`${API_BASE}/donations/progress`);
    const json = await res.json();
    if (!json.success) throw new Error(json.error?.message || 'Failed to fetch progress');
    return json.data;
  }

  // using demo data
  await delay(400);
  return { totalVerified: 3450, goal: 10000, percentage: 35 };
}

export async function submitDonation(data: SubmitDonationFormData): Promise<Donation> {
  if (BACKEND_AVAILABLE) {
    const formData = new FormData();
    formData.append('donorName', data.donorName);
    formData.append('donorEmail', data.donorEmail);
    formData.append('claimedAmount', String(data.claimedAmount));
    formData.append('receipt', data.receipt);
    const res = await fetch(`${API_BASE}/donations`, { method: 'POST', body: formData });
    const json = await res.json();
  ...
  return {
    ...
    receiptUrl: '/uploads/receipts/demo.jpg',
    status: 'pending',
...
export async function fetchPendingDonations(token?: string): Promise<Donation[]> {
  if (BACKEND_AVAILABLE) {
    const res = await fetch(`${API_BASE}/donations/pending`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const json = await res.json();
    if (!json.success) throw new Error(json.error?.message || 'Failed to fetch pending donations');
    return json.data;
  }

  // demo data — three receipts waiting for review
  await delay(700);
  return [
    {
      id: 'don-001',
      donorName: 'Ahmad Faris',
      donorEmail: 'ahmad.faris@graduate.utm.my',
      claimedAmount: 50,
      receiptUrl: 'https://placecats.com/200/280',
      status: 'pending',
      reviewedByUserId: null,
      reviewedAt: null,
    ...
    },
    {
      id: 'don-002',
      donorName: 'Nurul Ain',
      donorEmail: 'nurul.ain@utm.my',
      claimedAmount: 120,
      receiptUrl: 'https://placecats.com/200/281',
      status: 'pending',
      reviewedByUserId: null,
      reviewedAt: null,
    ...
    {
      id: 'don-003',
      donorName: 'Lee Chong Wei',
      donorEmail: 'lee.cw@graduate.utm.my',
      claimedAmount: 200,
      receiptUrl: 'https://placecats.com/200/282',
      status: 'pending',
      reviewedByUserId: null,
      reviewedAt: null,
...
export async function reviewDonation(
  ...
  if (BACKEND_AVAILABLE) {
    const res = await fetch(`${API_BASE}/donations/${id}/review`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
