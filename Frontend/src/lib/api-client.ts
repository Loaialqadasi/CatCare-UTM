...
function normalizeEmergency(raw: RawEmergency): EmergencyReport {
  return {
    ...
    proofNotes: raw.proofNotes,
    proofImageUrl: raw.proofImageUrl,
    proofSubmittedByUserId: raw.proofSubmittedByUserId != null ? String(raw.proofSubmittedByUserId) : raw.proofSubmittedByUserId,
    proofSubmittedAt: raw.proofSubmittedAt,
...
// ─── Map / Geocoding ───
// These helpers call the backend's /api/map/* endpoints, which proxy to
// Nominatim (OpenStreetMap) with rate limiting and a UTM-campus viewbox bias.
...
export async function geocodeAddress(query: string): Promise<GeocodeResult[]> {
  ...
  const json = await res.json();
  if (!json.success) throw new Error(json.error?.message || 'Geocoding failed');
...
export async function searchPlaces(query: string): Promise<PlaceResult[]> {
  ...
  const res = await apiFetch(url);
  const json = await res.json();
  if (!json.success) throw new Error(json.error?.message || 'Places search failed');
  return (json.data ?? []).map((item: any) => ({
