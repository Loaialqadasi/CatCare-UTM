...
import { escapeLike } from '../../shared/utils';
import { Cat, CatHealthStatus, CreateCatInput } from './cats.types';
...
// Map database row to cat domain model
...
// Build WHERE clause dynamically based on search and health status filters
...
export const catsRepository = {
  // Insert new cat profile into database
  ...
  // Check if cat exists by ID (for validation)
  async existsById(id: number): Promise<boolean> {
    const [rows] = await db.execute<RowDataPacket[]>('SELECT id FROM cats WHERE id = ? LIMIT 1', [id]);
    return rows.length > 0;
  },

  // Retrieve paginated list with optional search and health status filtering
  ...
  // Count total cats matching search and health status criteria
