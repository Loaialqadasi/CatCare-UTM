...
import { Cat, CatHealthStatus, CreateCatInput, CareHistoryEntry } from './cats.types.js';
...
export const catsRepository = {
  async create(input: CreateCatInput): Promise<Cat> {
    const { rows } = await db.query<CatRow>(
      `INSERT INTO cats (nickname, description, photo_url, location_name, latitude, longitude, health_status, ownership_tag, created_by_user_id)
       ...
       RETURNING id, nickname, description, photo_url, location_name, latitude, longitude, health_status, ownership_tag, created_by_user_id, created_at, updated_at`,
      [
        input.nickname,
  ...
  async findById(id: number): Promise<Cat | null> {
    const { rows } = await db.query<CatRow>(
      'SELECT id, nickname, description, photo_url, location_name, latitude, longitude, health_status, ownership_tag, created_by_user_id, created_at, updated_at FROM cats WHERE id = $1 LIMIT 1',
      [id]
    );
    if (rows.length === 0) {
      return null;
  ...
  async existsById(id: number): Promise<boolean> {
    const { rows } = await db.query('SELECT id FROM cats WHERE id = $1 LIMIT 1', [id]);
  ...
  async list(search?: string, healthStatus?: CatHealthStatus, limit = 10, offset = 0): Promise<Cat[]> {
    ...
    const query = `SELECT id, nickname, description, photo_url, location_name, latitude, longitude, health_status, ownership_tag, created_by_user_id, created_at, updated_at FROM cats ${whereClause} ORDER BY created_at DESC LIMIT $${params.length + 1} OFFSET $${params.length + 2}`;
  ...
  async listCursor(
    ...
    const query = `SELECT id, nickname, description, photo_url, location_name, latitude, longitude, health_status, ownership_tag, created_by_user_id, created_at, updated_at FROM cats ${whereClause} ORDER BY id DESC LIMIT $${params.length + 1}`;
